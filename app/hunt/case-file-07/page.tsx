...
import { EchoGuide } from '@/components/case-file-07/shared/EchoGuide'
...
export default function OperationDeadlightPage() {
  ...
  useEffect(() => {
    async function initSession() {
      try {
        const res = await fetch('/hunt/case-file-07/api/progress')
        const data = await res.json()
        if (!data.authenticated) {
          await fetch('/hunt/case-file-07/api/auth/session', {
            method: 'POST',
